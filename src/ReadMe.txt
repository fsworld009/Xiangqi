================================================================================
    MFC 程式庫 : Xiangqi 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 Xiangqi 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 Xiangqi 應用程式的檔案。
...
Xiangqi.h
    ...
    並宣告 CXiangqiApp 應用程式類別。

Xiangqi.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CXiangqiApp。
...
res\Toolbar.bmp
    ...
    Xiangqi.rc 的 IDR_MAINFRAME TOOLBAR 陣列，即可
    新增工具列的按鈕。
/////////////////////////////////////////////////////////////////////////////

應用程式精靈建立文件類型和檢視:

XiangqiDoc.h、XiangqiDoc.cpp - 文件
    這些檔案包含您的 CXiangqiDoc 類別。編輯這些檔案，即可新增您的特
    殊文件資料，並實作檔案儲存和載入 (透過 CXiangqiDoc::Serialize)。

XiangqiView.h、XiangqiView.cpp - 文件的檢視
    這些檔案包含您的 CXiangqiView 類別。
    CXiangqiView 物件是用來檢視 CXiangqiDoc 物件。
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 Xiangqi.pch 的先行編譯標頭 (PCH) 檔，
